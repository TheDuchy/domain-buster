...
def isIP(domain):
    ips = re.findall(r"^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}\
                    ([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])", domain)
    if len(ips) == 0:
        return 0
    else:
        return 1
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description='A tool for recursive domain busting')
    parser.add_argument('-d', '--domain', help='Target domain / file',
                        required=True)
    parser.add_argument('-t', '--timeout', default=2, type=int,
                        help='Timeout treshold (sec). ["2"]')
    parser.add_argument('-o', '--output', default='result.txt',
                        help='Name for the file output. ["result.txt"]')
